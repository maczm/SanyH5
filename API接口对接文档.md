...
- **物料二维码格式**：`物料编码|供应商|序列号:数量`；三段均不得为空，数量必须是大于 0 的数字（纯数字，不接受 `2x`/`1e3` 等），否则页面直接拦截、不调用 API-KC3
- **提交防重**：API-KC3 / API-KC7 请求在途时页面忽略重复触发（扫码枪连扫、连按回车只会提交一次）
- **换单与加载失败清态**：API-KC1 成功后立即清空上一单的关重件数据；API-KC2 失败时列表保持清空（不残留上一单卡片），避免旧订单关重件被当成新订单提交
...
- **更换页半完成保护**：API-KC4 移除成功而 API-KC7 保存失败时，该旧件行标记为「重试保存」（点击即用已拿到的 `oldGenealogyID` 重试 API-KC7，**不重复调用 API-KC4**）；此时改选其它旧件会被拦截，点「返回」需二次确认（旧件谱系已删，返回即放弃本次更换）
