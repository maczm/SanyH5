...
1. **需要管理员权限的操作，AI 不自己尝试解决**：包括但不限于 `sudo`、`apt install`、写工作区之外的文件/目录、系统级配置。
2. 遇到此类需求，AI 直接**给出可复制的命令**（附用途说明），由**用户在终端执行**；用户执行完告知后，AI 再验证结果。
3. AI 不得尝试绕过权限限制（如沙箱升级、改路径规避、`danger-full-access` 等），一律走"给命令 → 用户执行"流程。
