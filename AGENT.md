...
- **共享参数名照抄，不做"纠错"**：`assemblyMaterialCheck_getWipOrderNoInfo` 的入参 `serachKey` 按业务提供方原文保留（疑似 `searchKey` 拼写错误，**未经 Portal 侧确认不许改名**）
- **跨页共享常量必须同时改**：图片压缩参数（`MAX_IMAGE_WIDTH`/`MAX_IMAGE_HEIGHT`/`JPEG_QUALITY`，数值以页面代码为准）在 mom-packing 与 mom-nameplate-photo-upload 两页 `CONFIG` 中各有一份，改一处必须同步另一处
